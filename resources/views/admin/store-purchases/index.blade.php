    ...
    <div class="py-8">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    ...
                    <p class="mt-1 text-sm text-slate-500">
                        Tinjau data pembelian Toko/Kios ke DGW sebelum disetujui atau ditolak oleh Admin.
                    </p>
                </div>
            </div>
            ...
            <div class="mb-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                <form method="GET" action="{{ route('admin.store-purchases.index') }}">
                    <div class="grid grid-cols-1 items-end gap-4 md:grid-cols-4">
                        <div class="md:col-span-2">
                            ...
                            <input type="text"
                                   ...
                                   placeholder="Kode transaksi, invoice, nama toko..."
                        ...
                        <div class="flex gap-2">
                            ...
                            <x-ui.button href="{{ route('admin.store-purchases.index') }}" variant="secondary">Reset</x-ui.button>
                        </div>
                    </div>
                </form>
            </div>
                    ...
                    <table class="min-w-[1000px] w-full text-left text-sm">
                        ...
                        @forelse ($storePurchases as $purchase)
                            <tr class="border-t border-slate-100">
                                <td class="px-4 py-3 text-slate-700">{{ $purchase->transaction_code }}</td>
                                <td class="px-4 py-3 text-slate-700">{{ $purchase->invoice_number ?? '-' }}</td>
                                <td class="px-4 py-3 text-slate-700">{{ $purchase->transaction_date }}</td>
                                <td class="px-4 py-3 font-medium text-slate-900">{{ $purchase->store->store_name ?? '-' }}</td>
                                <td class="px-4 py-3 text-slate-700">Rp {{ number_format($purchase->total_amount, 0, ',', '.') }}</td>
                                <td class="px-4 py-3 text-slate-700">{{ number_format($purchase->total_store_points) }}</td>
                                <td class="px-4 py-3">
                                ...
                                <td class="px-4 py-3 text-right">
                                    <a href="{{ route('admin.store-purchases.show', $purchase) }}"
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="px-4 py-8 text-center text-slate-500">
                                    Belum ada transaksi pembelian Toko/Kios ke DGW.
                                </td>
                            </tr>
                        @endforelse
                        </tbody>
